...
export function setupAdventure(ctx){
  ...
  let layers={forest:true,water:true,roads:true,buildings:true,pois:true,labels:true,contours:true,waypoints:true,slope:false},alertSettings={enabled:false,threshold:100,sound:false},deviation={armed:false},audio=null,draft=null,waypoints=[],weatherBusy=false,weatherSerial=0;
  ...
  function startEditor(copy){
    ...
    const r=ctx.getRoute();if(!r){$('startDialog').showModal();return;}
  ...
  function updateDraft(){const count=draft?.reduce((n,s)=>n+s.length,0)||0;$('draftInfo').textContent=`${count} 點 · ${(routeLength(draft||[])/1000).toFixed(2)} km · 手動畫線`;$('saveDraft').disabled=count<2;$('undoPoint').disabled=!count;$('reverseDraft').disabled=count<2;map.setDraft(draft);}
  $('addPoint').onclick=()=>{if(!draft)return;const p=map.centerPoint();if(!validCoordinate(p)){ctx.toast('中心點超出支援範圍。');return;}if(!draft.length)draft.push([]);const last=draft.at(-1).at(-1);if(last&&distance(last,p)<1){ctx.toast('請移動地圖至少 1 m 再加點。');return;}draft.at(-1).push(p);updateDraft();};
  ...
  function finishEditor(){draft=null;waypoints=[];map.setDraft(null);$('editorBar').classList.add('hide');if(!ctx.getRoute())ctx.showRoutes();}
