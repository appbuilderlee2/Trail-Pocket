...
export function setupPackageManager(ctx){
  ...
  async function search(query,center){
    ...
    for(const item of installed.filter(x=>x.state==='ready')){
      ...
      let index=searchCache.get(file.local);if(!index){index=JSON.parse(new TextDecoder().decode(await files.read(file.local)));if(index.version!==1||!Array.isArray(index.entries))throw Error(`${item.name} 搜尋索引損壞`);searchCache.set(file.local,index);}
  ...
  async function routingGraph(start,end){
    ...
    const graphs=[];
    for(const item of selected){const file=item.files.find(x=>x.name==='routing.graph');if(!file?.local)continue;let graph=graphCache.get(file.local);if(!graph){graph=JSON.parse(new TextDecoder().decode(await files.read(file.local)));if(graph.version!==1||!graph.nodes||!graph.edges)throw Error(`${item.name} 步道路網損壞`);graphCache.set(file.local,graph);}graphs.push(graph);}
