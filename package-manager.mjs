...
const INDEX_URL = 'https://github.com/appbuilderlee2/Trail-Pocket/releases/download/maps-v4-current/sa-index.json';
...
export function setupPackageManager(ctx) {
  ...
  async function updateIndex(){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),8000);try{const response=await fetch(INDEX_URL,{cache:'no-store',signal:controller.signal});if(!response.ok)return;const index=await response.json();if(index.schema!==1||!Array.isArray(index.packages))throw Error('地圖包目錄格式無效');for(const item of index.packages){const manifest=validatePackageManifest(item);manifests.set(manifest.id,manifest);await catalog.put('manifests',manifest);}render();}finally{clearTimeout(timer);}}
