...
export function setupExplore(ctx){
  ...
  $('saveArea').onclick=async()=>{
    ...
    try{
      for(const endpoint of ['https://overpass.kumi.systems/api/interpreter','https://overpass-api.de/api/interpreter']){const attempt=new AbortController(),cancel=()=>attempt.abort(controller.signal.reason),attemptTimer=setTimeout(()=>attempt.abort('endpoint-timeout'),40000);controller.signal.addEventListener('abort',cancel,{once:true});try{const response=await fetch(endpoint,{method:'POST',body:new URLSearchParams({data:query}),credentials:'omit',signal:attempt.signal});data=validateMap(await readDownload(response,attempt.signal));break;}catch(e){error=e;if(controller.signal.aborted)throw e;$('areaProgress').textContent='服務暫時未完成，正嘗試備用服務…';}finally{clearTimeout(attemptTimer);controller.signal.removeEventListener('abort',cancel);}}
