...
export function setupExplore(ctx){
  ...
  function jump(p,units){ctx.pauseFollow();ctx.nav('map');map.center=project(p);map.units=units;map.draw();$('jumpDialog').close();}
  ...
  function status(){
    ...
    else{const b=map.viewBounds(),near=[...areas,...routeMaps].filter(m=>overlaps(m.bounds,b));$('exploreStatus').textContent=(navigator.onLine?'離線預覽':'目前離線')+` · 畫面涵蓋 ${near.length} 個已下載範圍。`+(near.some(m=>contains(m.bounds,b))?'':'範圍外可能留白；可到「離線下載」開啟已儲存區域。');$('retryTiles').classList.add('hide');}
    $('baseMode').value=mode;
  }
  ...
  function startSelection(){
    ...
    if(areaKm2(map.viewBounds(.15))>150)map.zoom(Math.sqrt(25/areaKm2(map.viewBounds(.15))));
  ...
  $('saveArea').onclick=async()=>{
    ...
    const timeout=setTimeout(()=>controller.abort('timeout'),90000);let data,error;
    try{
      for(const endpoint of ['https://overpass.kumi.systems/api/interpreter','https://overpass-api.de/api/interpreter']){try{const response=await fetch(endpoint,{method:'POST',body:new URLSearchParams({data:query}),credentials:'omit',signal:controller.signal});data=validateMap(await readDownload(response,controller.signal));break;}catch(e){error=e;if(controller.signal.aborted)throw e;$('areaProgress').textContent='服務暫時未完成，正嘗試備用服務…';}}
      if(!data)throw error;if(controller.signal.aborted)throw Error('下載已取消。');
      const record={id,name,bounds,data,downloaded:Date.now(),size:new Blob([JSON.stringify(data)]).size};$('areaProgress').textContent='正在儲存…';await store.put('areas',record);
  ...
  function renderAreas(){
    ...
    for(const a of areas){const card=el('article',undefined,'download-card');card.append(el('h3',a.name),el('p',`${areaKm2(a.bounds).toFixed(2)} km² · ${ctx.formatSize(a.size)} · ${new Date(a.downloaded).toLocaleString()}`));
