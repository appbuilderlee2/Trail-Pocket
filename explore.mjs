...
export function setupExplore(ctx){
  ...
  function renderAreas(){
    ...
    for(const a of areas){const card=el('article',undefined,'download-card');card.append(el('h3',a.name),el('p',`${areaKm2(a.bounds).toFixed(2)} km² · ${ctx.formatSize(a.size)} · ${a.terrain?'建築物、地標及 20 m 等高線':a.detailLevel==='建築物及地標'?'建築物及地標；無等高線':'舊版基本地形；重載可加入建築物及等高線'} · ${new Date(a.downloaded).toLocaleString()}`));
