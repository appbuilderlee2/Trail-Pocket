...
export function listMapMeta(){return new Promise((resolve,reject)=>{const list=[],r=db.transaction('maps').objectStore('maps').openCursor();r.onsuccess=()=>{const c=r.result;if(!c){resolve(list);return;}const {data,...meta}=c.value;list.push(meta);c.continue();};r.onerror=()=>reject(r.error);});}
