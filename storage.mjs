...
export function listMapMeta(name='maps'){return new Promise((resolve,reject)=>{const list=[],r=db.transaction(name).objectStore(name).openCursor();r.onsuccess=()=>{const c=r.result;if(!c){resolve(list);return;}const {data,contours,...meta}=c.value;list.push(meta);c.continue();};r.onerror=()=>reject(r.error);});}
