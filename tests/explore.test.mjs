...
import {terrainTiles,terrainZoom,terrariumElevation,contoursFromSamples} from '../terrain.mjs';
import {MAX_AREA_KM2,PACKAGE_LIMIT_BYTES,splitBounds,mergeMapChunks,assertStorageRoom} from '../offline-download.mjs';
...
test('large offline areas allow 500 square kilometres and split into safe queries',()=>{const allowed={west:138,east:138.48,south:-34.1,north:-34};assert.ok(validateArea(allowed)>400);assert.equal(MAX_AREA_KM2,500);assert.equal(PACKAGE_LIMIT_BYTES,128*1024*1024);const parts=splitBounds(allowed);assert.ok(parts.length>4);assert.ok(parts.every(p=>validateArea(p)<=102));assert.throws(()=>validateArea({west:138,east:138.6,south:-34.1,north:-34}),/500 km²/);});
test('large area splitter rejects missing bounds',()=>{assert.throws(()=>splitBounds(null),/500 km²/);});
test('chunk merge removes boundary duplicates and storage guard preserves headroom',async()=>{const data=mergeMapChunks([{elements:[{type:'way',id:1},{type:'node',id:2}]},{elements:[{type:'way',id:1},{type:'way',id:3}]}]);assert.deepEqual(data.elements.map(x=>x.id),[1,2,3]);await assertStorageRoom(1024,{estimate:async()=>({usage:0,quota:20*1024*1024})});await assert.rejects(()=>assertStorageRoom(20*1024*1024,{estimate:async()=>({usage:19*1024*1024,quota:20*1024*1024})}),/空間不足/);});
...
test('large contour downloads lower terrain zoom instead of failing',()=>{const bounds={west:138,east:139.8,south:-35.2,north:-34};assert.ok(terrainZoom(bounds)<=12);assert.doesNotThrow(()=>terrainTiles(bounds,terrainZoom(bounds)));});
...
test('all new static modules deploy and precache, online tiles are not precached',async()=>{
  ...
  for(const asset of ['offline-download.mjs','explore.mjs','explore-core.mjs','online-map.mjs','terrain.mjs','explore.css']){assert.ok(sw.includes("'./"+asset+"'"));assert.ok(workflow.includes(asset));}
