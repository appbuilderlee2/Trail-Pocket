...
import {selectPrimaryViewport,createGeoTransform,geoToPage,pointInFootprint,accuracyPixels} from '../geopdf-core.mjs';
import {parseGeoPdf} from '../geopdf-parser.mjs';

const main={bbox:[20,780,433,23],gpts:[-34.72754,138.80417,-34.65048,138.80620,-34.65139,138.85710,-34.72845,138.85511],lpts:[0,1,0,0,1,0,1,1],pageWidth:595,pageHeight:842,wkt:'GDA_1994_MGA_Zone_54'};
...
test('largest valid GeoPDF viewport is selected instead of an inset locator map',()=>{
  ...
  assert.throws(()=>selectPrimaryViewport([{bbox:[0,0,1,1],gpts:[],lpts:[]}]),/地理定位/);
});

test('GeoPDF homography places Para Wirra coordinates and GPS accuracy on the page',()=>{
  const transform=createGeoTransform(main),devils=[138.82,-34.68],page=geoToPage(transform,devils);
  assert.ok(page[0]>.1&&page[0]<.4);
  assert.ok(page[1]>.25&&page[1]<.65);
...
test('parser reads embedded VP Measure GPTS and LPTS data from a PDF',async()=>{
  const pdf=await PDFDocument.create(),page=pdf.addPage([595,842]),ctx=pdf.context;
  const measure=ctx.obj({Subtype:'GEO',GPTS:main.gpts,LPTS:main.lpts,GCS:{WKT:PDFString.of(main.wkt)}});
